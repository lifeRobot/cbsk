...
use cbsk_base::log;
use cbsk_base::tokio::sync::Mutex;
...
pub struct ZipPacker {
    /// packer lock<br />
    /// prevent errors caused by packaging duplicate files during log packaging
    lock: Mutex<()>,
    /// if pack end will call this function
    pub pack_end: Option<Box<dyn Fn(String) -> Pin<Box<dyn Future<Output=()> + Send>> + Sync + Send>>,
}
...
impl super::Packer for ZipPacker {
    ...
    async fn pack(&self, split_name: String, split_path: String, log_path: Arc<LogPath>) {
        ...
        if let Err(e) = self.try_pack(split_name, split_path, log_path.clone()).await {
    ...
    #[cfg(all(not(feature = "log_pack"), feature = "dir_pack"))]
    async fn pack(&self, _split_name: String, _split_path: String, log_path: Arc<LogPath>) {
        let lock = self.lock.lock().await;
        if let Err(e) = self.dir_pack(log_path).await {
            log::error!("pack log fail: {e:?}");
        }
        drop(lock);
    }
...
impl ZipPacker {
    ...
    pub fn pack_end(f: impl Fn(String) -> Pin<Box<dyn Future<Output=()> + Send>> + Sync + Send + 'static) -> Self {
        Self { lock: Mutex::default(), pack_end: Some(Box::new(f)) }
    }

    /// try pack to log
    #[cfg(feature = "log_pack")]
    async fn try_pack(&self, split_name: String, split_path: String, log_path: Arc<LogPath>) -> ZipResult<()> {
        // build zip file
        let zip_path = split_path.trim_end_matches(log_path.name_suffix.as_str());
        let zip_file = format!("{zip_path}.zip");
        self.file_try_pack(zip_file, split_name.as_str(), split_path.as_ref()).await
    }

    /// log file try pack
    async fn file_try_pack(&self, zip_file: String, log_name: &str, log_file: &Path) -> ZipResult<()> {
        ...
        cbsk_base::match_some_return!(self.pack_end.as_ref(),Ok(()))(zip_file).await;
        Ok(())
    }
}
...
impl ZipPacker {
    ...
    async fn dir_pack(&self, log_path: Arc<LogPath>) -> ZipResult<()> {
        ...
        for file in file_list {
            self.file_pack(file, log_path.as_ref()).await;
        }
        Ok(())
    }

    /// try pack file
    async fn file_pack(&self, file: DirEntry, log_path: &LogPath) {
        ...
        if file_name.ends_with(".zip") {
            if let Err(e) = self.zip_re_pack(file_name, log_path).await {
                log::error!("zip re pack fail: {e:?}");
            }
            return;
        }

        if let Err(e) = self.log_re_pack(file_name, log_path).await {
            log::error!("log re pack fail: {e:?}");
        }
    }

    /// zip re pack
    async fn zip_re_pack(&self, file_name: &str, log_path: &LogPath) -> ZipResult<()> {
        ...
        self.file_try_pack(zip_file, temp_name.as_str(), temp_path.as_path()).await
    }

    /// log re pack
    async fn log_re_pack(&self, file_name: &str, log_path: &LogPath) -> ZipResult<()> {
        // get log path and zip path etc
        ...
        if !file_name.ends_with(log_path.name_suffix.as_str()) {
            cbsk_base::match_some_return!(self.pack_end.as_ref(),Ok(()))(temp_file).await;
            return Ok(());
        }
        ...
        self.file_try_pack(zip_file, file_name, temp_path.as_path()).await
    }
}
